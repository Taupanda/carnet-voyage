...
export async function attachProfiles(sb, rows) {
...
export default function Social({ entryDate, onNeedLogin }) {
  ...
  async function toggleLike() {
    ...
    if (liked) {
      ...
      const { error } = await sb.from("likes").delete().eq("entry_date", entryDate).eq("user_id", user.id);
  ...
  async function delComment(id) {
    await supabaseBrowser().from("comments").delete().eq("id", id);
    load();
  }
        ...
        <div className="cmt-zone">
          ...
          {user ? (
            <div className="cmt-form">
              <Avatar p={profile} />
              <input className="input" placeholder="Écrire un commentaire…" value={text}
