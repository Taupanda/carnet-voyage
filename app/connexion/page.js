...
export default function Connexion() {
  ...
  const titles = { signin: "Se connecter", signup: "Créer un compte", forgot: "Mot de passe oublié" };

  return (
    <main className="container" style={{ paddingTop: 40, paddingBottom: 60, maxWidth: 420 }}>
      <h1 className="display" style={{ fontSize: 28, marginBottom: 6 }}>{titles[mode]}</h1>
      <p style={{ color: "var(--text2)", fontSize: 14.5, marginBottom: 24 }}>
        {mode === "forgot"
      ...
      {mode !== "forgot" && (
        <>
          ...
          <div className="sep"><span>ou par email</span></div>
        </>
      )}

      <label className="lbl">Email</label>
      <input className="input" type="email" autoComplete="email" placeholder="ton@email.fr"
      ...
      {mode !== "forgot" && (
        <>
          ...
          <input className="input" type="password"
            ...
            onKeyDown={(e) => e.key === "Enter" && submit()} style={{ marginBottom: 8 }} />
        </>
      )}
      ...
      <button className="btn" style={{ width: "100%", marginTop: 16 }} onClick={submit} disabled={busy}>
        {busy ? "…" : mode === "signup" ? "Créer mon compte" : mode === "forgot" ? "Envoyer le lien" : "Me connecter"}
      </button>

      <div className="switch">
        {mode === "signin" && (
          <>Pas encore de compte ?{" "}
            <button className="link-btn" onClick={() => { setMode("signup"); setErr(null); setInfo(null); }}>Créer un compte</button>
          </>
        )}
        {mode === "signup" && (
          <>Déjà un compte ?{" "}
            <button className="link-btn" onClick={() => { setMode("signin"); setErr(null); setInfo(null); }}>Se connecter</button>
          </>
        )}
        {mode === "forgot" && (
          <button className="link-btn" onClick={() => { setMode("signin"); setErr(null); setInfo(null); }}>← Retour à la connexion</button>
        )}
      </div>
    </main>
  );
