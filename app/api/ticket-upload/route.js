...
// Billets : PDF + images. Extension dérivée du type MIME (jamais du nom client).
const ALLOWED = {
  "application/pdf": "pdf",
  "image/jpeg": "jpg",
  "image/png": "png",
...
export async function POST(request) {
  ...
  const { error } = await db.storage.from("tickets").upload(path, buf, { contentType, upsert: false });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  // Nom d'affichage assaini (le vrai fichier est stocké sous `path`).
  const displayName = String(file.name || `billet.${ext}`).replace(/[\r\n]/g, "").slice(0, 80);
