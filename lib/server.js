...
// Le modèle écrit parfois de vrais retours à la ligne à l'intérieur d'une chaîne
// JSON — un résumé en plusieurs paragraphes — que JSON.parse refuse (« Bad
// control character in string literal »). On les réécrit en séquences
// d'échappement, uniquement à l'intérieur des chaînes : ailleurs ils sont permis.
function echapperControles(texte) {
  let sortie = "";
...
// des fences markdown, du texte parasite autour de l'objet ou des retours à la
// ligne bruts dans les chaînes.
export function extractJson(raw) {
  const cleaned = (raw || "").replace(/```json|```/g, "").trim();
  const start = cleaned.indexOf("{");
  const end = cleaned.lastIndexOf("}");
  const candidats = [cleaned];
  if (start !== -1 && end > start) candidats.push(cleaned.slice(start, end + 1));
  for (const candidat of candidats) {
    for (const texte of [candidat, echapperControles(candidat)]) {
      try {
